...
const UserCard = ({ user, currentUser, onConnectionUpdate }) => {
    const getInitialStatus = () => {
        if (currentUser.connections?.includes(user._id)) return 'connected';
        if (currentUser.sentRequests?.includes(user._id)) return 'pending';
        return 'connect';
    };
    ...
    return (
        <div className="bg-white p-4 rounded-lg shadow-sm border flex flex-col items-center text-center">
            <Link to={`/profile/${user._id}`}>
                <img
                    src={user.profile?.avatar?.url || 'https://placehold.co/80x80'}
                    alt={user.firstName}
