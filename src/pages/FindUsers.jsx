...
const UserCard = ({ user, currentUser, onConnectionUpdate }) => {
    // ✅ Connection status ab currentUser ki details se tay hoga
    ...
    const handleConnect = async () => {
        setConnectionStatus('pending'); // UI ko turant update karein
        try {
            ...
            // Parent component ko batayein ki state update karna hai
            onConnectionUpdate(user._id);
        } catch (error) {
            console.error(`Failed to send request to ${user.firstName}:`, error);
            setConnectionStatus('connect'); // Error hone par button ko wapas theek karein
        }
    };
...
const FindUsersPage = () => {
    // ✅ Outlet se currentUser ka poora object lein
    ...
    useEffect(() => {
        const fetchAllData = async () => {
            ...
            try {
                ...
                // Current user ki updated details fetch karein (sentRequests, etc.)
    ...
    // ✅ Connection request bhejne ke baad currentUser state ko update karein
